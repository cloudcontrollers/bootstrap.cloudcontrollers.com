...
echo "$(date) Bootstrap script4: Installing the s3tools repos to make sure we have the latest debian packages for s3cmd"
wget -O- -q http://s3tools.org/repo/deb-all/stable/s3tools.key | apt-key add -
wget -O/etc/apt/sources.list.d/s3tools.list http://s3tools.org/repo/deb-all/stable/s3tools.list
#
echo "$(date) Bootstrap script4: Editing sshd config to prevent time expiration logouts" 
...
EOF
#
echo "$(date) Bootstrap script4: Editing ulimit to override the default limit on file descriptors"
SYSCTL=/etc/sysctl.conf
cat << END >> $SYSCTL
fs.file-max = 200000
END
#
echo "$(date) Bootstrap script4: Running the sysctl command to override the default limit on file descriptors"
sysctl -p
#
echo "$(date) Bootstrap script4: Editing limit.conf to override the default limit on file descriptors"
...
ulimit -n 200000
#
echo "$(date) Bootstrap script4: Installing webmin"
apt-get update
...
echo "$(date) Bootstrap script4: Installing packages which enable web management, monitoring, outgoing smtp via postfix, ebs attachment scripts, s3 bucket management, Amazon Route53 DNS management, unzip and the locate command"
wget -O- -q http://s3tools.org/repo/deb-all/stable/s3tools.key | apt-key add -
wget -O/etc/apt/sources.list.d/s3tools.list http://s3tools.org/repo/deb-all/stable/s3tools.list
apt-get -y -qq update
...
echo "$(date) Bootstrap script4: Setting hostname"
echo -e "$LOCAL_IPV4 $HOSTNAME \n127.0.0.1 localhost"  > /etc/hosts
echo $HOSTNAME > /etc/hostname
hostname $HOSTNAME
#
#
echo "$(date) Bootstrap script4: Setting postfix hostname to actual hostname"
MAILNAME=/etc/mailname
cat << END > $MAILNAME
$HOSTNAME
END

echo "$(date) Bootstrap script4: Making directory for splash page"
mkdir -p /opt/data
